...
def ipl_predictor(current_score, wicketsfallen, overs_completed, scoreof_striker, scoreof_nonstriker):
   ...
   # custom input
   new_prediction = lin.predict((np.array(
      [[current_score, wicketsfallen, overs_completed, scoreof_striker, scoreof_nonstriker]])))
...
def predict_score():
   ...
   if format.get() == "IPL":
      score = ipl_predictor(int(entry1.get()), int(entry2.get()), float(
         entry3.get()), int(entry4.get()), int(entry5.get()))

      fLabel = Label(frame, text="The Predicted Score is " + str(score))
      fLabel.pack()
   elif format.get() == "ODI":
      score = odi_predictor(int(entry1.get()), int(entry2.get()), float(
         entry3.get()), int(entry4.get()), int(entry5.get()))
      ...
      score = t20_predictor(int(entry1.get()), int(entry2.get()), float(
         entry3.get()), int(entry4.get()), int(entry5.get()))
...
frame = LabelFrame(root, text="Score Predictor", padx=1,
                  borderwidth=1, bg="#b0b7c2", pady=1)
...
label2 = Label(frame, text="No of Wickets Fallen:").pack(
   anchor=W, padx=5, pady=5)
entry2 = Entry(frame, width=35)
entry2.pack()

label3 = Label(frame, text="No of overs Completed: ").pack(
   anchor=W, padx=5, pady=5)
entry3 = Entry(frame, width=35)
entry3.pack()

label4 = Label(frame, text="Score of the Stiker Batsman: ").pack(
   anchor=W, padx=5, pady=5)
entry4 = Entry(frame, width=35)
entry4.pack()

label5 = Label(frame, text="Score of the Non Stiker Batsman: ").pack(
   anchor=W, padx=5, pady=5)
entry5 = Entry(frame, width=35)
entry5.pack()

predict_button = Button(frame, text="Click to get the Predicted Score",
                        command=predict_score, padx=5, pady=5, borderwidth=3).pack()

another_prediction = Button(frame, text="Click here to Predict Again",
                           command=predicting_again, padx=5, pady=5).pack()
